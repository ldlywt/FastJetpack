...
# 分支解释
- 封装一对应分支：withLoading(https://github.com/ldlywt/FastJetpack/tree/withLoading)
- 封装二对应分支： Master（https://github.com/ldlywt/FastJetpack/tree/master ）
...
对于封装二，虽然解耦更彻底，但是关于Loading这里我觉得还是有遗憾。

试想一下：如果Activity中业务很多、逻辑复杂，存在很多个网络请求，如果都在需要网络请求的地方手动去`showLoading()`,然后在`observer()`中手动调用`stopLoading()`。

这样其实会导致一个顺序流程的割裂，因为网络请求开始前展示`showLoading()`，然后请求网络，结束后`stopLoading()`，这是一个完整的流程，不应该在代码中放到不同的地方。

以后代码量一多，以后维护起来，万一不小心删除了`showLoading()`或者`stopLoading()`，也容易导致问题。

还有就是每次都要手动调用这两个方法，麻烦。

最近有时间研究了下Kotlin flow，发现可以完美解决，Activity中不再需要手动`showLoading()`和`stopLoading()`。

ViewModel中,如果不需要loading，还是和封装二一样，如果需要loading，调用方法`launchWithLoading()`,这个方法接受2个回调，`requestBlock`：网络请求执行方法，`resultCallback`:结果的回调。
...
```
具体见分支：
> flow（<https://github.com/ldlywt/FastJetpack/tree/flow>）
